# 鸿蒙记账应用 - 项目架构说明文档

## 📋 项目概览

**项目名称**: HarmonyExpense (健康记账)  
**应用包名**: com.example.healthydiet  
**技术栈**: HarmonyOS + ArkTS + RelationalStore  
**架构模式**: 分层架构 (Layered Architecture)  
**数据库**: SQLite (通过 RelationalStore API)  
**版本**: 1.0.0

---

## 🏗️ 整体架构设计

### 架构分层

项目采用经典的**四层架构**设计，职责清晰，依赖单向：

```
┌─────────────────────────────────────────────────┐
│           Presentation Layer (表示层)            │
│              pages/ (UI组件)                     │
│         - Index.ets (首页)                       │
│         - AddBill.ets (添加账单)                 │
│         - CategoryManagement.ets (分类管理)      │
└─────────────────────────────────────────────────┘
                      ↓ 依赖
┌─────────────────────────────────────────────────┐
│          Business Logic Layer (业务层)           │
│              service/ (业务服务)                 │
│         - export/ (导出备份服务)                 │
│           · ExportService                        │
│           · ImportService                        │
│           · EncryptionModule                     │
└─────────────────────────────────────────────────┘
                      ↓ 依赖
┌─────────────────────────────────────────────────┐
│        Data Access Layer (数据访问层)            │
│              dao/ (数据访问对象)                 │
│         - UserDAO, AccountDAO, BillDAO           │
│         - CategoryDAO, BudgetDAO, TagDAO         │
│         - StatisticsDAO                          │
└─────────────────────────────────────────────────┘
                      ↓ 依赖
┌─────────────────────────────────────────────────┐
│      Infrastructure Layer (基础设施层)           │
│         database/ (数据库基础设施)               │
│         - DatabaseManager (数据库管理)           │
│         - IndexManager (索引管理)                │
│         - BatchQueryHelper (批量查询)            │
│         - CacheManager (缓存管理)                │
│         - PerformanceMonitor (性能监控)          │
│         - DatabaseConfigOptimizer (配置优化)     │
│                                                  │
│         model/ (数据模型)                        │
│         - User, Account, Bill, Category          │
│         - Budget, Tag, Statistics                │
└─────────────────────────────────────────────────┘
```

### 架构原则

#### 1. **单一职责原则 (SRP)**
- **DAO层**: 仅负责单表的CRUD操作，不包含业务逻辑
- **Service层**: 处理跨表业务逻辑和复杂操作
- **Database层**: 专注于数据库连接、事务、索引等基础设施

#### 2. **依赖倒置原则 (DIP)**
- 上层依赖下层的抽象接口，而非具体实现
- 所有DAO通过 `DatabaseManager.getDatabase()` 获取数据库实例
- 统一的错误处理和日志记录机制

#### 3. **开闭原则 (OCP)**
- 通过继承和组合扩展功能，而非修改现有代码
- 新增DAO只需实现标准接口，无需修改DatabaseManager

---

## 📦 核心模块详解

### 1. Model层 (数据模型)

**位置**: `entry/src/main/ets/model/`

#### 核心模型

| 模型 | 文件 | 职责 | 关键字段 |
|------|------|------|---------|
| User | User.ets | 用户信息 | userId, username, email, passwordHash |
| Account | Account.ets | 账户管理 | accountId, userId, name, type, balance |
| Category | Category.ets | 分类管理 | categoryId, userId, name, type, parentCategoryId, sortOrder |
| Bill | Bill.ets | 账单记录 | billId, userId, accountId, categoryId, amount, type |
| Budget | Budget.ets | 预算管理 | budgetId, userId, categoryId, amount, period |
| Tag | Tag.ets | 标签管理 | tagId, userId, name, usageCount |
| BillTag | BillTag.ets | 账单-标签关联 | billId, tagId |
| Statistics | Statistics.ets | 统计数据 | MonthlyStatistics, CategoryStatistics |

#### 设计特点

```typescript
// 所有模型遵循统一规范
export class Category {
  categoryId: number = 0
  userId: number = 0
  name: string = ''
  type: 'expense' | 'income' = 'expense'
  icon: string = '📦'
  color: string = '#1890FF'
  parentCategoryId: number = 0
  sortOrder: number = 0
  createdAt: string = ''
  updatedAt: string = ''
  is_deleted: number = 0

  // 构造函数支持完整初始化
  constructor(/* 11个参数 */) { }

  // 数据验证方法
  validate(): boolean {
    return this.userId > 0 && this.name.trim().length > 0
  }

  // JSON序列化支持
  toJSON(): object { }
  static fromJSON(json: any): Category { }
}
```

**关键设计决策**:
- ✅ 所有日期使用字符串格式 (避免Date对象兼容性问题)
- ✅ 布尔值使用数字 (1/0，符合SQLite习惯)
- ✅ 枚举值使用字符串字面量类型 (类型安全)
- ✅ 所有字段都有默认值 (避免undefined)

---

### 2. DAO层 (数据访问)

**位置**: `entry/src/main/ets/dao/`

#### DAO职责矩阵

| DAO | 主要功能 | 特殊能力 |
|-----|---------|---------|
| UserDAO | 用户CRUD、登录验证 | 邮箱唯一性检查 |
| AccountDAO | 账户CRUD、余额管理 | 批量操作、余额范围查询 |
| CategoryDAO | 分类CRUD、树形结构 | 递归查询、分类树构建、聚合统计 |
| BillDAO | 账单CRUD、外键验证 | 日期范围查询、分页查询 |
| BudgetDAO | 预算CRUD、活跃状态 | 周期查询、预算检查 |
| TagDAO | 标签CRUD、关联管理 | 热门标签、使用计数、账单-标签关联 |
| StatisticsDAO | 统计数据CRUD | 月度统计、分类统计 |

#### DAO标准模式

```typescript
export class CategoryDAO {
  // 1. 基础CRUD
  static async insert(category: Category): Promise<void>
  static async getById(userId: number, categoryId: number): Promise<Category | null>
  static async getAll(userId: number): Promise<Category[]>
  static async update(category: Category): Promise<void>
  static async softDelete(userId: number, categoryId: number): Promise<void>

  // 2. 批量操作
  static async bulkInsert(categories: Category[]): Promise<void>

  // 3. 业务查询
  static async getByType(userId: number, type: 'expense' | 'income'): Promise<Category[]>
  static async getChildren(userId: number, parentId: number): Promise<Category[]>

  // 4. 高级功能
  static async getCategoryTree(userId: number, type: string): Promise<CategoryTreeNode[]>
  static async aggregateByCategory(userId: number, startDate: string, endDate: string): Promise<CategoryAggregation[]>

  // 5. 辅助方法
  static async exists(categoryId: number): Promise<boolean>
  private static mapRowToCategory(resultSet: relationalStore.ResultSet): Category
  private static toError(message: string, err: any): Error
}
```

**关键设计模式**:
- ✅ 静态方法 (无需实例化)
- ✅ 统一错误处理 (toError方法)
- ✅ ResultSet资源管理 (finally块关闭)
- ✅ 事务支持 (批量操作)
- ✅ 参数验证 (validate调用)

---

### 3. Database层 (数据库基础设施)

**位置**: `entry/src/main/ets/database/`

#### 核心组件

##### 3.1 DatabaseManager (数据库管理器)

**职责**: 统一数据库初始化、连接管理、表创建

```typescript
export class DatabaseManager {
  private static db: relationalStore.RdbStore | null = null
  private static isInitialized: boolean = false

  // 单例模式初始化
  static async initDatabase(context: common.Context): Promise<void>

  // 获取数据库实例
  static getDatabase(): relationalStore.RdbStore

  // 创建所有表（按依赖顺序）
  private static async createAllTables(): Promise<void>

  // 事务封装
  static async transaction(fn: () => Promise<void>): Promise<void>
}
```

**表创建顺序** (遵循外键依赖):
1. users (独立表)
2. accounts, categories (依赖users)
3. bills (依赖accounts, categories)
4. budgets (依赖users, categories)
5. tags (依赖users)
6. bill_tags (依赖bills, tags)
7. monthly_statistics, category_statistics (统计表)

##### 3.2 IndexManager (索引管理器)

**职责**: 创建和管理数据库索引，优化查询性能

```typescript
export class IndexManager {
  // 创建所有优化索引
  static async createAllIndexes(): Promise<void>

  // 索引类型
  // - 复合索引: (user_id, type)
  // - 部分索引: WHERE is_deleted = 0
  // - 排序索引: (user_id, sort_order)
  // - 覆盖索引: (account_id, type, transaction_date, amount)
}
```

**索引策略**:
- ✅ 高频查询字段建索引
- ✅ 外键字段建索引
- ✅ 排序字段建索引
- ✅ 部分索引减少索引大小
- ✅ 复合索引遵循最左前缀原则

##### 3.3 BatchQueryHelper (批量查询助手)

**职责**: 优化批量操作性能

```typescript
export class BatchQueryHelper {
  // 批量插入 (事务 + 多行VALUES)
  static async bulkInsert<T>(items: T[], insertFn: (item: T) => Promise<void>): Promise<void>

  // 批量更新
  static async bulkUpdate<T>(items: T[], updateFn: (item: T) => Promise<void>): Promise<void>

  // IN查询优化
  static async queryByIds<T>(ids: number[], queryFn: (id: number) => Promise<T>): Promise<T[]>
}
```

##### 3.4 CacheManager (缓存管理器)

**职责**: 内存缓存，减少数据库访问

```typescript
export class CacheManager {
  private static cache: Map<string, CacheEntry> = new Map()
  private static readonly DEFAULT_TTL = 5 * 60 * 1000 // 5分钟

  static get<T>(key: string): T | null
  static set<T>(key: string, value: T, ttl?: number): void
  static clear(pattern?: string): void
}
```

**缓存策略**:
- ✅ 分类列表缓存 (高频读取)
- ✅ 用户信息缓存
- ✅ TTL过期机制
- ✅ 模式匹配清除

##### 3.5 PerformanceMonitor (性能监控)

**职责**: 监控查询性能，识别慢查询

```typescript
export class PerformanceMonitor {
  // 记录查询时间
  static async measureQuery<T>(name: string, fn: () => Promise<T>): Promise<T>

  // 获取性能报告
  static getReport(): PerformanceReport

  // 慢查询阈值: 100ms
}
```

##### 3.6 DatabaseConfigOptimizer (配置优化器)

**职责**: 优化数据库配置参数

```typescript
export class DatabaseConfigOptimizer {
  static async applyOptimizations(): Promise<void> {
    // WAL模式 (Write-Ahead Logging)
    await store.executeSql('PRAGMA journal_mode=WAL')

    // 同步模式
    await store.executeSql('PRAGMA synchronous=NORMAL')

    // 缓存大小 (40MB)
    await store.executeSql('PRAGMA cache_size=10000')

    // 临时存储在内存
    await store.executeSql('PRAGMA temp_store=MEMORY')

    // 启用外键约束
    await store.executeSql('PRAGMA foreign_keys=ON')
  }
}
```

---

### 4. Service层 (业务服务)

**位置**: `entry/src/main/ets/service/`

#### 导出备份服务

##### 4.1 ExportService (导出服务)

**职责**: 导出用户数据为JSON文件

```typescript
export class ExportService {
  static async exportUserData(
    userId: number,
    options: ExportOptions
  ): Promise<string> {
    // 1. 收集所有数据
    const data = await this.collectUserData(userId)

    // 2. 序列化为JSON
    const jsonData = JSON.stringify(data, null, 2)

    // 3. 计算校验和
    const checksum = await ChecksumHelper.generateChecksum(jsonData)

    // 4. 可选加密
    let finalData = jsonData
    if (options.encrypt && options.password) {
      finalData = await EncryptionModule.encryptData(jsonData, options.password)
    }

    // 5. 保存文件
    const filePath = await FileHelper.saveFile(finalData, options)

    return filePath
  }
}
```

##### 4.2 ImportService (导入服务)

**职责**: 导入备份数据，恢复用户数据

```typescript
export class ImportService {
  static async importUserData(
    filePath: string,
    password?: string
  ): Promise<ImportResult> {
    // 1. 读取文件
    let fileContent = await FileHelper.readFile(filePath)

    // 2. 解密 (如果需要)
    if (password) {
      fileContent = await EncryptionModule.decryptData(fileContent, password)
    }

    // 3. 校验和验证
    const isValid = await ChecksumHelper.verifyChecksum(fileContent)

    // 4. 解析JSON
    const data = JSON.parse(fileContent)

    // 5. 事务化导入
    await DatabaseManager.transaction(async () => {
      await this.importAccounts(data.accounts)
      await this.importCategories(data.categories)
      await this.importBills(data.bills)
      // ...
    })

    return { success: true, imported: stats }
  }
}
```

##### 4.3 EncryptionModule (加密模块)

**职责**: AES-256-GCM加密/解密

```typescript
export class EncryptionModule {
  // AES-256-GCM加密
  static async encryptData(plaintext: string, password: string): Promise<string>

  // AES-256-GCM解密
  static async decryptData(ciphertext: string, password: string): Promise<string>

  // PBKDF2密钥派生 (100,000次迭代)
  private static async deriveKey(password: string, salt: Uint8Array): Promise<CryptoKey>
}
```

**安全特性**:
- ✅ AES-256-GCM认证加密
- ✅ PBKDF2密钥派生
- ✅ 随机盐值和IV
- ✅ 防篡改检测

---

### 5. Pages层 (UI界面)

**位置**: `entry/src/main/ets/pages/`

#### 页面组件

| 页面 | 文件 | 功能 | 状态管理 |
|------|------|------|---------|
| 首页 | Index.ets | 账单列表、统计展示 | @State transactions, totalIncome, totalExpense |
| 添加账单 | AddBill.ets | 账单录入表单 | @State amount, category, account |
| 分类管理 | CategoryManagement.ets | 分类CRUD、树形展示 | @State categories, selectedCategory |

#### UI架构模式

```typescript
@Entry
@Component
struct Index {
  // 状态管理
  @State transactions: Array<Bill> = []
  @State allCategories: Array<Category> = []
  @State allAccounts: Array<Account> = []

  // 生命周期
  async aboutToAppear() {
    await this.loadData()
  }

  // 数据加载
  private async loadData() {
    this.transactions = await BillDAO.getAll(userId)
    this.allCategories = await CategoryDAO.getAll(userId)
    this.allAccounts = await AccountDAO.getAll(userId)
  }

  // 渲染
  build() {
    Column() {
      // UI组件
    }
  }
}
```

---

## 🔍 关键技术特性

### 1. 分类树形结构

**数据结构**: 多叉树 (N-ary Tree)

```typescript
interface CategoryTreeNode {
  category: Category
  children: CategoryTreeNode[]
}

// 树构建算法 (O(n)复杂度)
private static buildCategoryTree(categories: Category[]): CategoryTreeNode[] {
  const map = new Map<number, CategoryTreeNode>()  // 哈希表优化
  const roots: CategoryTreeNode[] = []

  // 初始化节点
  categories.forEach(cat => {
    map.set(cat.categoryId, { category: cat, children: [] })
  })

  // 建立父子关系
  categories.forEach(cat => {
    const node = map.get(cat.categoryId)
    if (cat.parentCategoryId === 0) {
      roots.push(node)
    } else {
      const parent = map.get(cat.parentCategoryId)
      parent?.children.push(node)
    }
  })

  return roots
}
```

**应用场景**:
- 分类层级展示
- 递归查询子分类
- 分类聚合统计

### 2. 标签系统

**多对多关系**: Bill ↔ Tag (通过bill_tags关联表)

```typescript
// 为账单添加标签
await TagDAO.addTagToBill(billId, tagId)

// 批量设置标签
await TagDAO.setTagsForBill(billId, [1, 2, 3])

// 查询账单的所有标签
const tags = await TagDAO.getTagsByBillId(billId)

// 标签聚合统计
const stats = await TagDAO.aggregateByTag(userId, startDate, endDate)
```

### 3. 索引优化策略

#### 复合索引
```sql
-- 账单日期范围查询 (最高频)
CREATE INDEX idx_bills_date 
ON bills (transaction_date DESC, bill_id DESC) 
WHERE is_deleted = 0;

-- 账户+日期复合索引
CREATE INDEX idx_bills_account_date 
ON bills (account_id, transaction_date DESC, type) 
WHERE is_deleted = 0;
```

#### 覆盖索引
```sql
-- 金额统计查询 (避免回表)
CREATE INDEX idx_bills_stat 
ON bills (account_id, type, transaction_date, amount) 
WHERE is_deleted = 0;
```

#### 部分索引
```sql
-- 只索引未删除数据
CREATE INDEX idx_categories_user_type 
ON categories (user_id, type) 
WHERE is_deleted = 0;
```

### 4. 批量操作优化

```typescript
// 批量插入 (事务 + 多行VALUES)
static async bulkInsert(bills: Bill[]): Promise<void> {
  const BATCH_SIZE = 100
  await store.beginTransaction()

  try {
    for (let i = 0; i < bills.length; i += BATCH_SIZE) {
      const batch = bills.slice(i, i + BATCH_SIZE)
      const placeholders = batch.map(() => '(?, ?, ?, ...)').join(', ')
      const sql = `INSERT INTO bills (...) VALUES ${placeholders}`
      await store.executeSql(sql, params)
    }
    await store.commit()
  } catch (error) {
    await store.rollBack()
    throw error
  }
}
```

### 5. 缓存机制

```typescript
// 分类列表缓存
static async getAll(userId: number): Promise<Category[]> {
  const cacheKey = `categories:${userId}`

  // 尝试从缓存获取
  let categories = CacheManager.get<Category[]>(cacheKey)
  if (categories) return categories

  // 从数据库查询
  categories = await this.queryFromDatabase(userId)

  // 写入缓存
  CacheManager.set(cacheKey, categories, 5 * 60 * 1000)  // 5分钟

  return categories
}
```

---

## 📊 数据流向

### 查询流程

```
User Action (UI)
    ↓
Page Component (@State)
    ↓
DAO Layer (static methods)
    ↓
CacheManager (check cache)
    ↓ (cache miss)
DatabaseManager (getDatabase)
    ↓
RelationalStore API
    ↓
SQLite Database
    ↓
ResultSet → Model Object
    ↓
Cache Update
    ↓
Return to Page
    ↓
UI Update (@State change)
```

### 插入流程

```
User Input (UI)
    ↓
Page Component (form data)
    ↓
Model Validation (validate())
    ↓
DAO Layer (insert method)
    ↓
Foreign Key Validation
    ↓
DatabaseManager (transaction)
    ↓
RelationalStore.executeSql()
    ↓
SQLite Database
    ↓
Cache Invalidation
    ↓
Success/Error Response
    ↓
UI Feedback
```

---

## 🔐 安全设计

### 1. 数据加密
- AES-256-GCM认证加密
- PBKDF2密钥派生 (100,000次迭代)
- 随机盐值和IV
- 防篡改检测 (GCM认证标签)

### 2. SQL注入防护
- 参数化查询 (PreparedStatement)
- 严格类型检查
- 输入验证

### 3. 数据完整性
- 外键约束
- CHECK约束
- UNIQUE约束
- 事务保证原子性

---

## 📈 性能优化

### 1. 数据库层面
- WAL模式 (Write-Ahead Logging)
- 索引优化 (复合索引、部分索引、覆盖索引)
- 批量操作 (事务 + 多行插入)
- 缓存大小优化 (40MB)

### 2. 应用层面
- 内存缓存 (CacheManager)
- 懒加载 (分页查询)
- 游标分页 (Cursor-based Pagination)
- 查询结果复用

### 3. 算法优化
- 树构建使用Map (O(n²) → O(n))
- 批量操作避免循环查询
- 递归深度限制

---

## 🧪 测试架构

### 测试分层

```
entry/ohosTest/ets/test/
├── model/          # 模型层测试 (6个)
│   ├── User.test.ets
│   ├── Account.test.ets
│   ├── Category.test.ets
│   ├── Bill.test.ets
│   ├── Budget.test.ets
│   └── Statistics.test.ets
│
├── dao/            # DAO层测试 (6个)
│   ├── UserDAO.test.ets
│   ├── AccountDAO.test.ets
│   ├── CategoryDAO.test.ets
│   ├── BillDAO.test.ets
│   ├── BudgetDAO.test.ets
│   └── StatisticsDAO.test.ets
│
├── pages/          # UI层测试 (3个)
│   ├── IndexPage.test.ets
│   ├── AddBillPage.test.ets
│   └── CategoryManagementPage.test.ets
│
└── services/       # 服务层测试 (1个)
    └── Transaction.test.ets
```

**测试覆盖**: 16个测试文件，覆盖核心功能

---

## 🚀 启动流程

```typescript
// EntryAbility.onCreate()
async onCreate() {
  // 1. 初始化所有DAO (已废弃，现在统一使用DatabaseManager)
  // await AccountDAO.initDatabase(context)
  // await CategoryDAO.initDatabase(context)
  // ...

  // 2. 统一初始化数据库 (推荐)
  await DatabaseManager.initDatabase(context)

  // 3. 应用配置优化
  await DatabaseConfigOptimizer.applyOptimizations()

  // 4. 创建所有表
  await DatabaseManager.createAllTables()

  // 5. 创建索引
  await IndexManager.createAllIndexes()

  // 6. 插入模拟数据 (开发环境)
  await this.insertMockPrerequisites()
}
```

---

## 📝 架构优势

### ✅ 优点

1. **清晰的分层**: 职责明确，易于维护
2. **统一管理**: DatabaseManager统一数据库初始化
3. **性能优化**: 索引、缓存、批量操作
4. **安全可靠**: 加密、事务、外键约束
5. **可扩展性**: 新增功能只需添加DAO和Service
6. **测试友好**: 分层测试，覆盖全面

### ⚠️ 改进空间

1. **DAO初始化冗余**: EntryAbility中仍保留旧的DAO初始化代码
2. **缓存策略**: 缓存失效策略可以更精细
3. **错误处理**: 可以统一错误码和错误类型
4. **日志系统**: 可以引入结构化日志
5. **配置管理**: 可以外部化配置参数

---

## 📚 技术栈总结

| 层次 | 技术 | 用途 |
|------|------|------|
| UI层 | ArkTS + ArkUI | 声明式UI框架 |
| 业务层 | TypeScript | 业务逻辑 |
| 数据层 | RelationalStore API | 关系型数据库 |
| 存储层 | SQLite | 本地数据库 |
| 加密 | AES-256-GCM | 数据加密 |
| 测试 | Hypium | 单元测试框架 |

---

**文档版本**: 1.0  
**编写日期**: 2025-11-12  
**适用版本**: HarmonyExpense 1.0.0  
**维护者**: 开发团队
