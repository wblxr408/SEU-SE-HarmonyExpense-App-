...
本文档详细说明基于 ArkTS 和 HarmonyOS 关系型数据库的索引优化策略和批量查询实现方案，旨在提升应用数据库性能和响应速度。
...
#### 2.1.1 选择性原则
- 为高选择性字段创建索引（唯一值多的字段）
- 避免为低选择性字段创建索引（如性别、状态等）

#### 2.1.2 查询频率原则
- 优先为高频查询字段创建索引
- WHERE、JOIN、ORDER BY 子句中的字段
...
export class BillDAO {
  ...
  static async bulkInsert(bills: Bill[]): Promise<void> {
    ...
    try {
      ...
      for (const bill of bills) {
        if (!bill.validate()) {
          throw new Error(`无效账单数据: ${JSON.stringify(bill)}`);
        }
      ...
      await store.commit();
      console.log(`[BillDAO] 批量插入成功: ${bills.length} 条`);
    } catch (error) {
  ...
  /**
   * 批量插入（优化版 - 使用 VALUES 多行插入）
   * 注意：SQLite 有参数数量限制（通常 999 个）
   */
  static async bulkInsertOptimized(bills: Bill[]): Promise<void> {
    if (bills.length === 0) return;
    
    const store = DatabaseManager.getDatabase();
    const BATCH_SIZE = 100; // 每批次插入数量
...
static async bulkUpdate(bills: Bill[]): Promise<void> {
  ...
  try {
    ...
    await store.commit();
    console.log(`[BillDAO] 批量更新成功: ${bills.length} 条`);
  } catch (error) {
    await store.rollBack();
    console.error('[BillDAO] 批量更新失败:', error);
    throw error;
  }
}
```
...
static async bulkSoftDelete(billIds: number[]): Promise<void> {
  ...
  try {
    ...
    console.log(`[BillDAO] 批量删除成功: ${billIds.length} 条`);
  } catch (error) {
    console.error('[BillDAO] 批量删除失败:', error);
    throw error;
  }
}
```
...
#### 3.4.3 游标分页（Cursor-based Pagination）

```typescript
/**
 * 游标分页查询（适用于实时数据流）
 * @param userId 用户ID
 * @param cursor 游标（上次查询的最后一条记录ID）
 * @param limit 查询数量
 */
...
export class DatabaseManager {
  static async optimizeDatabase(): Promise<void> {
    ...
    try {
      ...
      // 设置缓存大小（单位：页，默认页大小 4KB）
      await store.executeSql('PRAGMA cache_size=10000'); // 约 40MB
      ...
      console.error('[DatabaseManager] 数据库优化失败:', error);
    }
  }
}
```
...
export class DatabaseMaintenance {
  ...
  static async analyze(): Promise<void> {
    ...
    try {
      ...
      console.log('[Maintenance] 统计信息分析完成');
    } catch (error) {
      console.error('[Maintenance] 分析失败:', error);
    }
  }
  ...
  static async vacuum(): Promise<void> {
    ...
    try {
      ...
      console.log('[Maintenance] 数据库清理完成');
    } catch (error) {
  ...
  static async reindex(): Promise<void> {
    ...
    try {
      ...
      console.log('[Maintenance] 索引重建完成');
    } catch (error) {
      console.error('[Maintenance] 重建失败:', error);
    }
  }
}
```
